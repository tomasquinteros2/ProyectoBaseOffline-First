// File: micro/microservicio_proveedor/src/main/java/micro/microservicio_proveedor/sync/OneDriveListener.java
...
public class OneDriveListener {
    ...
    public static void exportChange(Object entity, String action) {
        ...
        try {
            ...
            // incluir idPart en el nombre del archivo (sanitizar caracteres no permitidos)
            String safeId = idPart.replaceAll("[^A-Za-z0-9\\-@]", "");
            String fileName = String.format("%s_%s_%s_%s_%s.json",
                    time, syncProps.getNodeId(), dto.getEntityType(), safeId, UUID.randomUUID().toString().substring(0, 8));
