package micro.microservicio_tipo_producto.sync.initial;
...
import micro.microservicio_tipo_producto.sync.SyncContext;
import micro.microservicio_tipo_producto.sync.SyncProperties;
...
public class InitialSnapshotService {
    ...
    private Optional<Object> findExistingEntity(SyncableRepository<?, ?> repo, Object entity) {
        if (repo instanceof TipoProductoRepository tipoRepo && entity instanceof TipoProducto tipo) {
            String nombre = tipo.getNombre();
            if (nombre != null && !nombre.isBlank()) {
                return tipoRepo.findByNombreIgnoreCase(nombre.trim()).map(found -> (Object) found);
            }
        }
        try {
