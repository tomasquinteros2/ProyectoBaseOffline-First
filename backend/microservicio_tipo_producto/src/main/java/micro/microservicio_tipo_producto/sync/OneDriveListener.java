...
public class OneDriveListener {
    ...
    private static volatile micro.microservicio_tipo_producto.sync.SyncProperties props;
    ...
    public static void setSyncProperties(micro.microservicio_tipo_producto.sync.SyncProperties p) {
        props = p;
        if (p != null) {
    ...
    public static void exportChange(Object entity, String action) {
        if (micro.microservicio_tipo_producto.sync.SyncContext.isSyncing()) return;

        // Obtener props (fallback similar al original)
        micro.microservicio_tipo_producto.sync.SyncProperties syncProps = getSyncProperties();
        if (syncProps == null) {
        ...
        try {
            micro.microservicio_tipo_producto.sync.SyncEventDTO dto = new micro.microservicio_tipo_producto.sync.SyncEventDTO();
    ...
    private static micro.microservicio_tipo_producto.sync.SyncProperties getSyncProperties() {
        if (props != null) return props;
        try {
            micro.microservicio_tipo_producto.sync.BeanUtil bu = micro.microservicio_tipo_producto.sync.BeanUtil.getBean(micro.microservicio_tipo_producto.sync.BeanUtil.class);
            micro.microservicio_tipo_producto.sync.SyncProperties p = micro.microservicio_tipo_producto.sync.BeanUtil.getBean(micro.microservicio_tipo_producto.sync.SyncProperties.class);
            props = p;
            return p;
        } catch (Exception e) {
